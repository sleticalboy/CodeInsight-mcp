...
fn cli_resolves_rust_crate_and_super_use_imports() {
    ...
    assert!(
        deps["dependencies"]
            ...
            .iter()
            .any(|dependency| {
                dependency["target"] == "crate::support::nested::*"
                    && dependency["resolved_file"] == "src/support/nested.rs"
    ...
    assert!(
        root_callees
            ...
            .unwrap()
            .iter()
            .any(|call| { call["callee"] == "record" && call["callee_file"].is_null() })
    );
...
"#,
    ...
    let task_path_request = serde_json::json!({
        ...
        "params": {
            ...
            "arguments": {
                "root": fixture.path(),
                "task": "inspect 'src/main.ts:12:4' task path without backend call",
                "token_budget": 500,
                "backend": {
                    "provider": "codebase-memory-mcp",
                    "project": "fixture",
...
#[test]
fn mcp_stdio_agent_route_bounds_ambiguous_backend_symbols_in_compact_mode() {
    let fixture = fixture_project();
    let mut workers = String::new();
    for index in 1..=10 {
        workers.push_str(&format!(
    ...
    assert!(
        route["response_budget"]["omitted_metadata_fields"]
            .as_u64()
            .unwrap()
            > 0
    );
