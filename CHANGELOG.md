...
- C# simple type bindings such as `UserService users`, `App.Services.UserService backupUsers`, or alias-based `Repo repoUsers` can now provide `callee_file` hints for instance calls like `users.Find` when the type is fully qualified, found through a local `using` or namespace declaration, or expanded through a local `using` alias.
