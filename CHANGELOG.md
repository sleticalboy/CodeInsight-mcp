...
- C# simple type bindings such as `UserService users`, `App.Services.UserService backupUsers`, alias-based `Repo repoUsers`, or `var createdUsers = new UserService()` can now provide `callee_file` hints for instance calls like `users.Find` when the type is explicit, constructed locally, or expanded through a local `using` alias.
