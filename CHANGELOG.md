...
- C# simple type bindings such as `UserService users` or `App.Services.UserService backupUsers` can now provide `callee_file` hints for instance calls like `users.Find` when the type is fully qualified or found through a local `using` or namespace declaration.
