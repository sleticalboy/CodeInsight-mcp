...
- C# simple type bindings such as `UserService users` can now provide `callee_file` hints for instance calls like `users.Find` when the type is found through a local `using` or namespace declaration.
