...
top_type_relation_target = metrics.fetch("top_type_relation_target", "")
top_type_relation_target = "-" if top_type_relation_target.empty?
type_relation_filter = metrics.fetch("type_relation_recommendation_kinds", []).join(",")
type_relation_filter = "-" if type_relation_filter.empty?
...
content << line("| Type-relation edges | `#{metrics.fetch("type_relation_edges", 0)}` |")
content << line("| Top type-relation target | `#{top_type_relation_target}` |")
content << line("| Type-relation graph filter | `#{type_relation_filter}` |")
