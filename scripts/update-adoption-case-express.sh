...
REPO_URL="${CODEINSIGHT_ADOPTION_CASE_REPO_URL:-https://github.com/expressjs/express.git}"
REPO_ROOT="${CODEINSIGHT_ADOPTION_CASE_ROOT:-}"
COMMIT_REF="${CODEINSIGHT_ADOPTION_CASE_COMMIT:-}"
WORK_DIR="${CODEINSIGHT_ADOPTION_CASE_WORK_DIR:-/tmp/codeinsight-adoption-case-express}"
OUTPUT_FILE="${CODEINSIGHT_ADOPTION_CASE_OUTPUT:-$ROOT_DIR/docs/adoption-case-express.md}"
TASK="${CODEINSIGHT_ADOPTION_CASE_TASK:-understand express application routing behavior}"
...
usage() {
  cat <<'EOF'
usage: scripts/update-adoption-case-express.sh [options]

Refreshes docs/adoption-case-express.md from a live adoption-comparison run.

Options:
  --root PATH           Existing Express checkout. Skips clone.
  --repo-url URL        Repository URL. Default: https://github.com/expressjs/express.git.
  --commit REF          Commit, branch, or tag to check out after clone.
  --work-dir PATH       Temporary work directory. Default: /tmp/codeinsight-adoption-case-express.
  --output PATH         Output Markdown path. Default: docs/adoption-case-express.md.
...
\`\`\`bash
scripts/update-adoption-case-express.sh --commit $commit_full
\`\`\`
...
git clone --depth 1 $REPO_URL /tmp/codeinsight-case-express
