...
write_markdown() {
  {
    ...
    echo "## Results"
    echo
    echo "| Task | Seed strategy | First file | First seed | Companion | Lines | Reduction | Tokens | Impact |"
    echo "| --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: |"
    jq -r '.tasks[] |
      "| \(.task) | `\(.seed_strategy)` | `\(.first_file)` | `\(.first_seed_value)` | `\((.companion_entrypoint // "") as $value | if $value == "" then "-" else $value end)` | `\(.selected_lines)/\(.total_lines)` | `\(.line_reduction)` | `\(.estimated_tokens)` | `\(.risk_level) / \(.impacted_files)` |"' \
      "$SUMMARY_JSON"
    echo
