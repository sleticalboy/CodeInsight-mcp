...
main() {
  ...
  require_pattern docs/task-routing-expectations/requests.tsv \
    $'understand requests adapter test coverage\ttests/test_adapters\\.py' \
  ...
  require_jq docs/public-task-routing-matrix-summary.json \
    '.cases[] | select(.case == "requests") | .routes[] | select(.task == "understand requests adapter test coverage" and .first_file == "tests/test_adapters.py" and (.first_reading_focus | contains("regression coverage")) and (.first_reading_question | contains("assertions")) and (.first_reading_question | contains("fixtures")))' \
  ...
  require_pattern docs/public-task-routing-matrix.md \
    'express: 15 tasks, first files index\.js, lib/application\.js, lib/express\.js, lib/request\.js, lib/response\.js' \
    "public task routing matrix express summary"
  require_pattern docs/public-task-routing-matrix.md \
    'gin: 18 tasks, first files auth\.go, binding/default_validator\.go, binding/default_validator_test\.go, binding/json\.go, binding/query\.go, context\.go, gin\.go, recovery\.go, render/render\.go, routergroup\.go, tree\.go' \
    "public task routing matrix gin summary"
  require_pattern docs/public-task-routing-matrix.md \
    'flask: 16 tasks, first files src/flask/app\.py, src/flask/blueprints\.py, src/flask/cli\.py, src/flask/config\.py, src/flask/helpers\.py, src/flask/logging\.py, src/flask/sansio/blueprints\.py, src/flask/sessions\.py, src/flask/templating\.py, src/flask/views\.py, src/flask/wrappers\.py, tests/test_testing\.py' \
