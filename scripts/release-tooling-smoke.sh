...
main() {
  ...
  smoke_run_step "$SMOKE_TOTAL" 14 "benchmark artifact smoke smoke" "$ROOT_DIR/scripts/benchmark-artifact-smoke-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 15 "release pretag check smoke" "$ROOT_DIR/scripts/release-pretag-check-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 16 "release metadata summary smoke" "$ROOT_DIR/scripts/release-metadata-summary-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 17 "release workflow guard smoke" "$ROOT_DIR/scripts/release-workflow-guard-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 18 "release tag preflight smoke" "$ROOT_DIR/scripts/release-tag-preflight-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 19 "release evidence summary smoke" "$ROOT_DIR/scripts/release-evidence-summary-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 20 "release dry run smoke" "$ROOT_DIR/scripts/release-dry-run-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 21 "archive release evidence smoke" "$ROOT_DIR/scripts/archive-release-evidence-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 22 "release handoff summary smoke" "$ROOT_DIR/scripts/release-handoff-summary-smoke.sh"
  smoke_run_step "$SMOKE_TOTAL" 23 "release notes draft smoke" "$ROOT_DIR/scripts/release-notes-draft-smoke.sh"
