...
| First MCP call onboarding changed | `scripts/mcp-first-call-smoke.sh --summary-json /tmp/codeinsight-mcp-first-call.json` | Compact JSON proof for `agent_route`, first context file, task-path seed evidence, route quality, read-less metrics, selection rank, reading-question handoff, continuation summary, reading-plan order, suggested-tool handoff, impact status, blocked no-seed/no-context/unindexed-path recovery actions, and saved artifacts. |
...
| Public route expectations changed | `scripts/public-task-routing-matrix.sh --case express --root express=/path/to/express --require-roots --min-route-quality-score 70` | Checked-in public repository expectation files aggregated into one route-quality summary, with optional route-quality score gates. Defaults include pinned Express, FastAPI, Flask, Gin, Requests, Streamlit, and Wouter cases. Use `--require-roots` with local caches to avoid clone/fetch during evidence refreshes. Use `scripts/public-task-routing-matrix-smoke.sh` for a deterministic no-network contract check. |
| Public route snapshot changed | `scripts/update-public-task-routing-matrix.sh --check` | Checked-in public route-quality snapshot freshness. Add `--min-route-quality-score 70` when intentionally refreshing a gated public evidence snapshot, and pass `--require-roots` plus one `--root NAME=PATH` per case for offline/cached refreshes. Use `scripts/update-public-task-routing-matrix-smoke.sh` for a deterministic no-network contract check. |
...
| codebase-memory bridge changed | `scripts/codebase-memory-backend-evidence-smoke.sh` | Converts raw, MCP `structuredContent`, or JSON-RPC-wrapped exports from `get_code_snippet`, `search_graph`, `search_code`, `query_graph`, `trace_path`, and `get_architecture` into `agent_route --backend-evidence`, then verifies backend agreement appears in `route_quality` and backend/local conflicts require `compare_backend_route_before_edits`. |
| codebase-memory bridge report changed | `scripts/codebase-memory-bridge-report-smoke.sh` | Consumes `backend_evidence` plus raw `agent-route` JSON and emits agreement/conflict summary artifacts, including the agent-facing route action and warning count. |
| codebase-memory bridge cohort changed | `scripts/codebase-memory-bridge-cohort-summary-smoke.sh` | Aggregates multiple bridge report summaries into backend-route agreement rates, per-status counts, first-file match rates, agent-facing route actions, warning counts, and conflict review gates. |
| codebase-memory bridge cohort report changed | `scripts/codebase-memory-bridge-cohort-report-smoke.sh` | Batch-generates per-task bridge reports plus the aggregate cohort from a TSV manifest, preserving backend-route agreement status and agent-facing route actions in the cohort output. |
| External Beta handoff changed | `scripts/external-beta-cohort-report-smoke.sh` | Packages existing External Beta trial summaries into a cohort summary, fix queue, handoff README, ready-to-file handoff issue, machine-readable manifest, and optional printed snippet. |
...
repository multi-prompt route-quality gates, including optional
`--min-route-quality-score` thresholds,
...
`codebase-memory-bridge-cohort-report-smoke.sh` when changing the manifest-based
batch flow that creates per-task reports before aggregation. Use
