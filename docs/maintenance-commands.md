...
| First MCP call onboarding changed | `scripts/mcp-first-call-smoke.sh --summary-json /tmp/codeinsight-mcp-first-call.json` | Compact JSON proof for `agent_route`, first context file, reading-plan order, suggested-tool handoff, impact status, and saved artifacts. |
| First MCP call Actions summary changed | `scripts/mcp-first-call-step-summary-smoke.sh` | Checks the Actions Summary section for selected files, first context file, first reading file, reading-plan order, suggested-tool handoff, continuation timing, impact status, and artifact link. |
