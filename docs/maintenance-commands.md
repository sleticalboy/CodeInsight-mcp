...
| First MCP call onboarding changed | `scripts/mcp-first-call-smoke.sh` | Compact JSON proof for `agent_route`, selected files, execution plan, suggested tool, and impact status. |
| First MCP call failure messaging changed | `scripts/mcp-first-call-failure-smoke.sh` | Fast checks for `[binary]` and `[mcp_server]` failure prefixes. |
