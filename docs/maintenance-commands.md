...
| codebase-memory bridge changed | `scripts/codebase-memory-backend-evidence-smoke.sh` | Converts exported `get_code_snippet`, `search_graph`, `search_code`, `query_graph`, `trace_path`, and `get_architecture` JSON into `agent_route --backend-evidence`, then verifies backend agreement appears in `route_quality` and backend/local conflicts require `compare_backend_route_before_edits`. |
