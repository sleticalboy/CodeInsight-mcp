...
| First MCP call onboarding changed | `scripts/mcp-first-call-smoke.sh --summary-json /tmp/codeinsight-mcp-first-call.json` | Compact JSON proof for `agent_route`, first context file, selection rank, continuation summary, reading-plan order, suggested-tool handoff, impact status, and saved artifacts. |
...
| Task alias or seed ordering changed | `scripts/task-routing-matrix-smoke.sh` | Temporary fixture proving routing, authentication, settings, and startup prompts choose the matching first file. |
...
changes touch framework routing. Use `task-routing-matrix.sh` for real
repository multi-prompt route-quality checks, and `task-routing-matrix-smoke.sh`
