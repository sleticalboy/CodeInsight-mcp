...
| codebase-memory bridge cohort changed | `scripts/codebase-memory-bridge-cohort-summary-smoke.sh` | Aggregates multiple bridge report summaries into first-file match rates, candidate coverage, agent-facing route actions, warning counts, and conflict review gates. |
| codebase-memory bridge cohort report changed | `scripts/codebase-memory-bridge-cohort-report-smoke.sh` | Batch-generates per-task bridge reports plus the aggregate cohort from a TSV manifest, preserving agent-facing route actions in the cohort output. |
