...
| First MCP call onboarding changed | `scripts/mcp-first-call-smoke.sh --summary-json /tmp/codeinsight-mcp-first-call.json` | Compact JSON proof for `agent_route`, first context file, read-less metrics, selection rank, reading-question handoff, continuation summary, reading-plan order, suggested-tool handoff, impact status, blocked no-seed handling, and saved artifacts. |
