...
3. context_pack
   ...
   routing_decision_read_less: 99.4%, 163.5x
   ...
   blind_first_read_lines: 86982
   routed_first_read_lines: 532
   selected_lines: 532
   source_lines_avoided: 86450
   line_reduction: 99.4%
   read_less_ratio: 163.5x
   ...
   reading_plan_reason: Read this step to answer: Which seed selection, reading-plan handoff, or read-less evidence controls the agent first-read workflow here? If deeper evidence is needed, call file_outline. Selection reason: Selected for high relevance via seed_file: Seed file defines symbol agent_route; matched task keywords: agent, route; Seed file defines symbol read_agent_route_backend_evidence; matched task keywords: agent, route; evidence mix: seed file x9, call graph x1
   selection_reason: Selected for high relevance via seed_file: Seed file defines symbol agent_route; matched task keywords: agent, route; Seed file defines symbol read_agent_route_backend_evidence; matched task keywords: agent, route; evidence mix: seed file x9, call graph x1
   route_reason: selected 2 files, 11 ranges, and 2 reading-plan steps within the token budget; read src/tools.rs first (candidate rank 1) via inspect_seed_file, use file_outline when deeper evidence is needed; first omitted candidate src/mcp.rs (candidate rank 3, reason token_budget_exhausted) can be revisited via run_omitted_candidate_context_pack using context_pack after selected context; continuation run_omitted_candidate_context_pack
...
4. impact_analysis
   ...
   paths: 50
   suggested_checks: 4
   route_reason: after selected context is read, pre-edit impact check estimated 9 impacted files at high risk, including 8 call-related files, 3 dependency-related files, 50 call paths, and 0 dependency paths
...
[Evidence summary]
Blind first-read baseline: 86982 source lines.
Routed first-read: 532 source lines across 2 files.
Read less: avoided 86450 source lines, 163.5x less text before follow-up tools.
...
agent_route selected 532/86982 source lines (99.4% reduction) across 2 files.
...
6. routing_decision summarizes the same choice: seed=task_match:src/tools.rs, first_file=src/tools.rs, rank=1, read_less=99.4%/163.5x.
7. route_quality is high (100/100) from 23 evidence signals; recommended_action=read_selected_context_then_use_continuation_if_needed.
...
16. The selected context avoided 86450 source lines (99.4%, 163.5x less text); selected 2 files, 11 ranges, and 2 reading-plan steps within the token budget; read src/tools.rs first (candidate rank 1) via inspect_seed_file, use file_outline when deeper evidence is needed; first omitted candidate src/mcp.rs (candidate rank 3, reason token_budget_exhausted) can be revisited via run_omitted_candidate_context_pack using context_pack after selected context; continuation run_omitted_candidate_context_pack
17. Selection evidence: candidate rank 1; Selected for high relevance via seed_file: Seed file defines symbol agent_route; matched task keywords: agent, route; Seed file defines symbol read_agent_route_backend_evidence; matched task keywords: agent, route; evidence mix: seed file x9, call graph x1
...
19. impact_analysis reports high risk across 9 impacted files with 4 suggested checks; after selected context is read, pre-edit impact check estimated 9 impacted files at high risk, including 8 call-related files, 3 dependency-related files, 50 call paths, and 0 dependency paths
