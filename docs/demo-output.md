...
3. context_pack
   ...
   routing_decision_read_less: 99.4%, 155.2x
   ...
   blind_first_read_lines: 82571
   routed_first_read_lines: 532
   selected_lines: 532
   source_lines_avoided: 82039
   line_reduction: 99.4%
...
[Evidence summary]
Blind first-read baseline: 82571 source lines.
Routed first-read: 532 source lines across 1 files.
Read less: avoided 82039 source lines, 155.2x less text before follow-up tools.
...
agent_route selected 532/82571 source lines (99.4% reduction) across 1 files.
...
3. context_pack selected 1 files and 8 ranges, then produced 1 reading-plan steps.
...
6. routing_decision summarizes the same choice: seed=task_match:src/tools.rs, first_file=src/tools.rs, rank=1, read_less=99.4%/155.2x.
...
16. The selected context avoided 82039 source lines (99.4%, 155.2x less text); selected 1 files, 8 ranges, and 1 reading-plan steps within the token budget; read src/tools.rs first (candidate rank 1) via inspect_seed_file, use file_outline when deeper evidence is needed; first omitted candidate src/main.rs (candidate rank 2, reason token_budget_exhausted) can be revisited via run_omitted_candidate_context_pack using context_pack after selected context; continuation run_omitted_candidate_context_pack
