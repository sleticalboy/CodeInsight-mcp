...
3. context_pack
   ...
   routing_decision_read_less: 99.4%, 164.8x
   ...
   blind_first_read_lines: 87513
   routed_first_read_lines: 531
   selected_lines: 531
   source_lines_avoided: 86982
   line_reduction: 99.4%
   read_less_ratio: 164.8x
...
[Evidence summary]
Blind first-read baseline: 87513 source lines.
Routed first-read: 531 source lines across 1 files.
Read less: avoided 86982 source lines, 164.8x less text before follow-up tools.
...
agent_route selected 531/87513 source lines (99.4% reduction) across 1 files.
...
6. routing_decision summarizes the same choice: seed=task_match:src/tools.rs, first_file=src/tools.rs, rank=1, read_less=99.4%/164.8x.
...
16. The selected context avoided 86982 source lines (99.4%, 164.8x less text); selected 1 files, 10 ranges, and 1 reading-plan steps within the token budget; read src/tools.rs first (candidate rank 1) via inspect_seed_file, use file_outline when deeper evidence is needed; first omitted candidate src/main.rs (candidate rank 2, reason token_budget_exhausted) can be revisited via run_omitted_candidate_context_pack using context_pack after selected context; continuation run_omitted_candidate_context_pack
