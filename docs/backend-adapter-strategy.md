...
Candidate-producing exports can be ordered page arrays; the bridge unwraps each
page independently and aggregates page latency. It accepts at most sixteen pages
and four nested `result` wrappers per tool, rejects candidates outside the
project root, and deduplicates files before spending the 64-item per-tool budget.
