...
- Rust local `use crate::`, `use self::`, and `use super::` imports, including grouped and nested grouped members with aliases, can provide `callee_file` hints for scoped calls such as `audit::record()` and direct imported calls when the resolved local file contains the called symbol.
