...
- Same-repository workspace package `exports` resolution supports JSON-compatible `package.json` workspaces and common `pnpm-workspace.yaml` package lists with exact-directory, single-segment wildcard, and recursive `**` package patterns.
...
- External `imports` targets, catalog metadata expansion, glob negation, and multi-wildcard path segment syntax are not modeled yet.
...
- Add broader catalog metadata expansion and advanced workspace edge-case handling such as glob negation.
