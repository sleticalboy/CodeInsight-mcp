...
- Go same-module, local replacement, and `go.work` workspace imports can provide `callee_file` hints for package-qualified calls such as `auth.Login()` and explicit import aliases when a unique top-level function with that name exists anywhere in the resolved package directory.
