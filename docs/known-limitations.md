...
- Package metadata fallback supports root package specifiers through local relative `module`, `main`, `types`, and `typings` targets, skipping external, absolute, or non-string field values, plus package subpaths resolved as package-relative files or index files.
