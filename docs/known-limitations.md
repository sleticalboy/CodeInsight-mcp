...
- C# same-repository alias and static `using` directives can provide `callee_file` hints for qualified calls such as `Audit.Record()` and static imported calls such as `ClampName()` when the resolved local file contains the called symbol; namespace declarations can provide hints for same-namespace class-qualified calls such as `LocalFormatter.Normalize()`; simple, `this.`-qualified, awaited async, generic method, nullable explicit type, array type, common single-argument collection type, null-conditional, null-forgiving, target-typed `new`, fully-qualified, using-alias, and local `new` type bindings can provide hints for instance calls such as `users.Find()`, `this.users.Find()`, `await users.FindAsync()`, `users.FindAs<string>()`, `maybeUsers.Find()`, `servicePool[0].Find()`, `listUsers[0].Find()`, `users?.Find()`, `users!.Find()`, `targetUsers.Find()`, `backupUsers.Find()`, `repoUsers.Find()`, and `createdUsers.Find()`; `this.LocalTag()` is normalized for same-file call graph matching, while `base.LocalTag()` and chained property calls such as `users.Profile.Load()` are recorded but inheritance-chain and member-property target resolution are not modeled. C# null-conditional calls embedded inside large expression chains may be conservatively skipped when Tree-sitter exposes only a broad outer invocation, avoiding false combined callees.
