...
- Obvious local string bindings inside JavaScript and TypeScript `require(...)` / `import(...)` calls are resolved when they can be reduced to a file-local string target, but non-literal dynamic `import()`, external dynamic import handlers, package-based config `extends`, pnpm/yarn-specific workspace protocols, and broader bundler-specific resolution are not modeled yet.
