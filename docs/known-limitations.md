...
- Dependency package `exports` resolution supports nearest `node_modules` packages with exact, single-wildcard, or multi-wildcard mappings, configurable common condition priority, array fallback targets, explicit `null` disabled subpaths, matched conditional `null` branches, and common `browser` string/object remaps.
