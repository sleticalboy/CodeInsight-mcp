...
- File seed ranges include header/import context plus primary top-level symbols. Task-matching seed symbols get a small same-file ordering boost, large same-score merged ranges are capped, and oversized seed ranges can be shortened to fit small budgets. If no primary symbols are found, `context_pack` falls back to the first 80 lines.
