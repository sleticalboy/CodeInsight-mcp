...
- Obvious local string bindings inside JavaScript and TypeScript `require(...)` / `import(...)` calls are resolved when they can be reduced to a file-local string target, including simple concatenation and template literal interpolation, but non-literal dynamic `import()`, external dynamic import handlers, package-based config `extends`, pnpm/yarn-specific workspace protocols, and broader bundler-specific resolution are not modeled yet.
- Python absolute imports resolve obvious repository-root `.py` files and package directories through `__init__.py`, and relative `from .` / `from ..` imports resolve nearby package files with member-target fallback such as `.support.audit` to `support/audit.py` or `support.py`, plus package directories through `__init__.py`; absolute and relative `from ... import name as alias` forms, including parenthesized multi-line imports and submodule aliases, can provide direct imported-call hints; third-party imports are not resolved. Go import paths resolve same-module packages declared by the nearest `go.mod`, local relative `replace` targets, and modules listed by a local relative `go.work use` when they remain inside the indexed repository, preferring ordinary implementation files over package `doc.go` and `_test.go` files; standard library, third-party, remote or absolute workspace targets, and `vendor` paths are not resolved to files yet. C# `using` directives resolve obvious same-repository `.cs` files under common roots such as `src`, including alias/static using targets and namespace directories with representative files; namespace declarations stay unresolved in the dependency graph but can provide `callee_file` hints for same-namespace class-qualified calls; simple type bindings such as `UserService users`, `UserService targetUsers = new()`, `App.Services.UserService backupUsers`, `Repo repoUsers`, or `var createdUsers = new UserService()` can provide instance-call hints when the type is explicit, constructed locally, or expanded through a local `using` alias; `System.*` and `Microsoft.*` imports are not resolved. Java imports resolve obvious same-repository class files under common source roots such as `src/main/java`, including static member imports that can fall back to the containing class file; wildcard imports stay unresolved in the dependency graph but can provide `callee_file` hints for explicit class-qualified calls, and package declarations can provide hints for same-package class-qualified calls. PHP namespace `use` imports resolve obvious same-repository files under common roots such as `src`, including `App\` PSR-4 style paths, grouped use declarations, and function imports when a matching file exists. Ruby `require_relative` imports resolve obvious neighboring or parent-relative `.rb` files, with or without an explicit `.rb` suffix; gem-style `require` imports are not resolved. Rust `mod` declarations prefer sibling `foo.rs` before `foo/mod.rs`, and `crate::`, `self::`, and `super::` use paths resolve obvious local module files such as `src/foo.rs`, nested module files, or `src/foo/mod.rs`; external crate imports are not resolved.
...
- Direct type-relation ranges are included for recognized local inheritance or interface edges, currently covering C# `base_type` dependencies, Java, TypeScript/JavaScript, and PHP declaration-level `extends` / `implements` dependencies, Ruby `class Child < Parent` dependencies, and Rust `impl Trait for Type` dependencies that can be mapped back to an indexed class, interface, trait, or struct symbol.
...
- Rust local `use crate::`, `use self::`, and `use super::` imports, including grouped and nested grouped members with aliases or glob members, can provide `callee_file` hints for scoped calls such as `audit::record()` and direct imported calls when the resolved local file contains the called symbol. Module aliases can continue through indexed child modules for calls such as `helpers::nested::tool()`. Local `pub use`, restricted-visibility `pub(...) use`, aliased re-exports, and glob re-exports can forward those hints through one facade module, while explicit alias re-exports, a first-layer glob followed by an explicit alias, or an unambiguous second-layer glob can continue through a second facade module to the implementation file. Glob re-exports resolve only when the final symbol is unique across local glob targets. Fully qualified local calls such as `crate::support::audit::record()`, `self::nested::tool()`, and `super::support::helper()` resolve against indexed module files without requiring a `use` declaration. Glob imports can resolve unique unqualified member calls, while importing a module does not make that module's members available as unqualified calls.
- Go same-module, local replacement, and `go.work` workspace imports can provide `callee_file` hints for package-qualified calls such as `auth.Login()` and explicit import aliases when a unique top-level function with that name exists anywhere in the resolved package directory. Dot imports can resolve unique unqualified top-level function calls across the package, while blank imports never provide call hints.
...
- C# same-repository alias and static `using` directives can provide `callee_file` hints for qualified calls such as `Audit.Record()` and static imported calls such as `ClampName()` when the resolved local file contains the called symbol; fully-qualified static member calls such as `App.Support.AuditLog.Record()` and `App.Support.MathUtil.ClampName()` can also resolve by file path; explicit `using` aliases take precedence over same-namespace class names for calls such as `Audit.Record()`, and `using static` targets take precedence over same-named symbols reachable only through ordinary namespace `using` imports; namespace declarations can provide hints for same-namespace class-qualified calls such as `LocalFormatter.Normalize()`, and same-namespace classes are preferred over same-named classes reachable through namespace `using` imports; simple, `this.`-qualified, awaited async, generic method, nullable explicit type, array type, common single-argument collection type, common dictionary value type, null-conditional, null-forgiving, target-typed `new`, `Lazy<T>.Value`, `var new Lazy<T>`, `Task<T>.Result`, `ValueTask<T>.Result`, `var new Task<T>`, `var new ValueTask<T>`, wrapper-plus-collection variable bindings such as `Task<List<UserService>>` and `Lazy<Dictionary<string, UserService>>`, fully-qualified, using-alias, local `new`, temporary `new` including object/collection initializers and indexers, direct base-class, explicit interface type, and one-hop same-repository property return type bindings can provide hints for instance, base, and property calls such as `users.Find()`, `this.users.Find()`, `await users.FindAsync()`, `users.FindAs<string>()`, `maybeUsers.Find()`, `servicePool[0].Find()`, `listUsers[0].Find()`, `usersById[id].Find()`, `users?.Find()`, `users!.Find()`, `targetUsers.Find()`, `lazyUsers.Value.Find()`, `inferredLazyUsers.Value.Find()`, `taskUsers.Result.Find()`, `valueTaskUsers.Result.Find()`, `inferredTaskUsers.Result.Find()`, `inferredValueTaskUsers.Result.Find()`, `taskListUsers.Result[0].Find()`, `lazyMappedUsers.Value[id].Find()`, `backupUsers.Find()`, `repoUsers.Find()`, `createdUsers.Find()`, `new UserService().Find()`, `new UserService { }.Find()`, `new List<UserService> { users }[0].Find()`, `new Dictionary<string, UserService> { [id] = users }[id].Find()`, `new Lazy<UserService>(() => users).Value.Find()`, `new Task<UserService>(() => users).Result.Find()`, `new ValueTask<UserService>(users).Result.Find()`, `new System.Lazy<App.Services.UserService>(() => users).Value.Find()`, `directory.Find()`, `base.BaseTag()`, `base.RootTag()`, `users.Profile.Load()`, `users.ExternalProfile.Load()`, `new UserService { }.ExternalProfile.Load()`, `new App.Services.UserService().ExternalProfile.Load()`, `new App.Services.UserService { }.ExternalProfile.Load()`, `new List<UserService> { users }[0].ExternalProfile.Load()`, `new Lazy<UserService>(() => users).Value.ExternalProfile.Load()`, `new System.Lazy<App.Services.UserService>(() => users).Value.ExternalProfile.Load()`, `maybeUsers?.ExternalProfile.Load()`, `maybeUsers!.ExternalProfile.Load()`, `lazyUsers.Value.ExternalProfile.Load()`, `inferredLazyUsers.Value.ExternalProfile.Load()`, `taskUsers.Result.ExternalProfile.Load()`, `valueTaskUsers.Result.ExternalProfile.Load()`, `inferredTaskUsers.Result.ExternalProfile.Load()`, `inferredValueTaskUsers.Result.ExternalProfile.Load()`, `taskListUsers.Result[0].ExternalProfile.Load()`, `lazyMappedUsers.Value[id].ExternalProfile.Load()`, `backupUsers.ExternalProfile.Load()`, `repoUsers.ExternalProfile.Load()`, `createdUsers.ExternalProfile.Load()`, `targetUsers.ExternalProfile.Load()`, `servicePool[0].ExternalProfile.Load()`, `listUsers[0].ExternalProfile.Load()`, and `usersById[id].ExternalProfile.Load()`; direct base-class targets can resolve through same-namespace/using paths or fully-qualified forms such as `class AuthController : App.Controllers.BaseController`; overloaded methods are indexed by name and calls such as `users.Find(42)`, `users.Find(id, "active")`, `users.Find(id, includeDisabled: true)`, `users.Find(id, includeDisabled: true, scope: "admin")`, and `users.FindAs<int>(id, includeDisabled: true)` can point to the containing file, but signature-level overload selection is not modeled; explicit interface variables such as `IUserDirectory directory` can point to the interface file, but concrete implementation dispatch is not modeled, so implementation-only property chains such as `directory.ImplementationProfile.Load()` remain unresolved; extension methods such as `FormatForDisplay(this UserService users, ...)` can provide `callee_file` hints for instance-style extension calls such as `users.FormatForDisplay(id)`, `this.users.FormatForDisplay(id)`, `maybeUsers?.FormatForDisplay(id)`, `listUsers[0].FormatForDisplay(id)`, `lazyUsers.Value.FormatForDisplay(id)`, and temporary receiver calls such as `new UserService().FormatForDisplay(id)` when the extension namespace is imported and the receiver type is bound; bare nested generic collections such as `List<Dictionary<string, UserService>>` are not treated as direct type bindings; C# direct type-relation parsing records each comma-separated base or interface target from declarations such as `class Child : Base, IInterface`, and one-hop inherited base members such as `base.RootTag()` can resolve through the direct base type relation; inherited members beyond that one-hop base edge are skipped or recorded without resolving; `LocalTag()` and `this.LocalTag()` are normalized for same-file call graph matching, while multi-hop property chains such as `users.Profile.Metadata.Display()` are not modeled. C# null-conditional calls embedded inside large expression chains, including property chains and extension-style calls, may be conservatively skipped when Tree-sitter exposes only a broad outer invocation, avoiding false combined callees such as `ClampName.users.Find`.
...
- Extend blocked/empty-state contracts beyond `blocked_no_seed`,
  `blocked_invalid_seed`, and explicit-seed `blocked_no_context` so agents
