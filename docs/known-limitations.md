...
- C# same-repository alias and static `using` directives can provide `callee_file` hints for qualified calls such as `Audit.Record()` and static imported calls such as `ClampName()` when the resolved local file contains the called symbol; namespace declarations can provide hints for same-namespace class-qualified calls such as `LocalFormatter.Normalize()`; simple, `this.`-qualified, awaited async, generic method, nullable explicit type, array type, common single-argument collection type, common dictionary value type, null-conditional, null-forgiving, target-typed `new`, fully-qualified, using-alias, local `new`, direct base-class, and explicit interface type bindings can provide hints for instance and base calls such as `users.Find()`, `this.users.Find()`, `await users.FindAsync()`, `users.FindAs<string>()`, `maybeUsers.Find()`, `servicePool[0].Find()`, `listUsers[0].Find()`, `usersById[id].Find()`, `users?.Find()`, `users!.Find()`, `targetUsers.Find()`, `backupUsers.Find()`, `repoUsers.Find()`, `createdUsers.Find()`, `directory.Find()`, and `base.BaseTag()`; direct base-class targets can resolve through same-namespace/using paths or fully-qualified forms such as `class AuthController : App.Controllers.BaseController`; overloaded methods are indexed by name and calls such as `users.Find(id, includeDisabled: true)` can point to the containing file, but signature-level overload selection is not modeled; explicit interface variables such as `IUserDirectory directory` can point to the interface file, but concrete implementation dispatch is not modeled; extension methods such as `FormatForDisplay(this UserService users, ...)` are indexed, but instance-style extension calls such as `users.FormatForDisplay(id)` are recorded without resolving to the extension method file; non-whitelisted generic wrappers such as `Lazy<UserService>` and nested generic collection wrappers are not treated as type bindings; direct base-class parsing uses only the first `class Child : Base, IInterface` entry, so interface inheritance and inherited members beyond that direct base edge such as `base.RootTag()` are recorded but not resolved; `this.LocalTag()` is normalized for same-file call graph matching, while chained property calls such as `users.Profile.Load()` or `this.users.Profile.Load()` are recorded but inheritance chains beyond a direct base-class edge and member-property target resolution are not modeled. C# null-conditional calls embedded inside large expression chains may be conservatively skipped when Tree-sitter exposes only a broad outer invocation, avoiding false combined callees.
