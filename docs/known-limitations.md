...
- File-level `source` and `reason` report the dominant selected source among `seed_file`, `symbol_definition`, `reference`, `call_graph`, `semantic`, and `dependency`. Range-level `source` reports each selected range's source.
