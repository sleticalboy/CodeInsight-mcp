...
- Direct type-relation ranges are included for recognized local inheritance or interface edges, currently covering C# `base_type` dependencies, Java and TypeScript/JavaScript declaration-level `extends` / `implements` dependencies, and Rust `impl Trait for Type` dependencies that can be mapped back to an indexed class, interface, trait, or struct symbol.
