...
| You want a multi-task route quality check | `scripts/task-routing-matrix.sh /path/to/repo` | Runs routing/auth/settings/startup prompts and writes a Markdown/JSON matrix with first selected file, seed strategy, line reduction, token estimate, and impact preview. |
...
| You need adoption comparison evidence | `scripts/adoption-comparison.sh /path/to/repo --output-dir /tmp/codeinsight-adoption-comparison` | A blind-read vs routed-first-read report with source lines avoided, read-less ratio, seed strategy, first reading question, selection rank, and continuation next action. |
