...
| You want a multi-task route quality check | `scripts/task-routing-matrix.sh /path/to/repo --expect-file ./route-expectations.tsv` | Runs routing/auth/authorization/settings/startup/persistence/debug/coverage/API handler/billing/frontend/background job/documentation/middleware prompts and writes a Markdown/JSON matrix with first selected file, seed strategy, line reduction, token estimate, impact preview, and optional expected-file gates. |
