...
4. Pick the validation that matches your adoption stage:
   ...
   | Task routing matrix | `scripts/task-routing-matrix.sh /path/to/repo --task "understand routing behavior" --task "understand authentication behavior" --task "understand application settings" --expect "understand routing behavior=src/router.ts"` | You want a multi-task first-read matrix showing first selected file, seed strategy, line reduction, token estimate, impact preview, and optional expected-file gates for one repository. |
